...
public class Cnd extends AbstractQuery implements Query {
    ...
    protected static Cnd of(Class cls) {
        Cnd dsl = new Cnd();
        dsl.jdbcTemplate = SpringContextHolder.getBean0(JdbcTemplate.class);
        dsl.clazz = cls;
        return dsl;
    }
    ...
    public <T> List<T> select(Class resultType, String... columns) {
        ...
        if (resultType.isAssignableFrom(Map.class)) {
            ...
            list = this.jdbcTemplate.query(targetSql, paras, new BeanPropertyRowMapper<T>(resultType));
        }
        return list;
    }
    ...
    public int delete() {
        ...
        //先清除，避免执行出错后无法清除
        clear();
        int row = this.jdbcTemplate.update(targetSql, paras);
        return row;
    }
    ...
    public long count() {
        ...
        //先清除，避免执行出错后无法清除
        clear();
        Long result = this.jdbcTemplate.queryForObject(targetSql, Long.class, paras);
        return result;
    }
    ...
    private <T> int insert(T t, boolean ignoreNull) {
        ...
        try {
            return jdbcTemplate.update(stringBuilder.toString(), params.toArray());
        } catch (Exception e) {
            log.error("插入操作失败", e);
