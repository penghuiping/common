 ...
 */
public abstract class CndJpa extends AbstractNewQuery implements Query {

    private static final Logger log = LoggerFactory.getLogger(CndJpa.class);

    JdbcOperations jdbcOperations = null;

    DbType dbType;

    public static CndJpa of(Class cls, DbType dbType, JdbcOperations jdbcOperations) {
        CndJpa dsl = null;
        switch (dbType) {
            case MYSQL:
                dsl = new CndMysqlJpa(cls, jdbcOperations);
                break;
            case ORACLE:
                dsl = new CndOracleJpa(cls, jdbcOperations);
                break;
            default:
                dsl = new CndMysqlJpa(cls, jdbcOperations);
                break;
    ...
    public CndJpa condition() {
        CndJpa dsl = null;
        switch (dbType) {
            case MYSQL:
                dsl = new CndMysqlJpa(this.clazz, this.jdbcOperations);
                break;
            case ORACLE:
                dsl = new CndOracleJpa(this.clazz, this.jdbcOperations);
                break;
            default:
                dsl = new CndMysqlJpa(this.clazz, jdbcOperations);
                break;
        }
        return dsl;
    }
    ...
    public <T> List<T> select(Class resultType, String... columns) {
        ...
        if (resultType.isAssignableFrom(Map.class)) {
            ...
            list = this.jdbcOperations.query(targetSql, paras, new JpaModelRowMapper<T>(resultType));
        }
        return list;
    }
    ...
    @Override
    public CndJpa having(QueryCondition condition) {
        // 去除叠加条件中的WHERE
    ...
    @Override
    public CndJpa groupBy(String column) {
    ...
    @Override
    public CndJpa orderBy(String orderBy) {
    ...
    @Override
    public CndJpa asc(String column) {
        OrderBy orderByInfo = this.getOrderBy();
    ...
    @Override
    public CndJpa desc(String column) {
        OrderBy orderByInfo = this.getOrderBy();
    ...
    @Override
    public CndJpa join(Class<?> model, String column) {
     ...
     */
    @Override
    public CndJpa limit(long startRow, long pageSize) {
     ...
     * @return
     */
    public CndJpa andSearchParamBuilder(SearchParamBuilder searchParamBuilder) {
