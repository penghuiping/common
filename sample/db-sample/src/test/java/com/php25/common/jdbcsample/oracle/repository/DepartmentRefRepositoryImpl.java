...
public class DepartmentRefRepositoryImpl implements DepartmentRefRepository {
    ...
    public List<DepartmentRef> findByCustomerId(Long customerId) {
        return db.getBaseSqlExecute().select(db.from(DepartmentRef.class).whereEq("customer_id", customerId).select());
    }
    ...
    public void deleteByCustomerIds(List<Long> customerIds) {
        db.getBaseSqlExecute().delete(db.from(DepartmentRef.class).whereIn("customer_id", customerIds).delete());
    }
